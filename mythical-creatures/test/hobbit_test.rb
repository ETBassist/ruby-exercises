...
class HobbitTest < Minitest::Test
  ...
  def test_hobbit_has_the_ring_if_its_name_is_frodo
    ...
    frodo = Hobbit.new("Frodo") # create a hobbit named Frodo
    sam = Hobbit.new("Sam") # create a second hobbit named Sam
    frodo.get_ring
    assert frodo.has_ring? # check that .has_ring? for Frodo returns true
    refute sam.has_ring? # check that .has_ring? for Sam returns false
  end

  def test_hobbits_are_short

    hobbit = Hobbit.new("Geordie") # create a hobbit
