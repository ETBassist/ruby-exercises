...
class PirateTest < Minitest::Test
  ...
  def test_a_pirate_has_booty
    ...
    bob = Pirate.new("Bob the Broke") # create a pirate
    assert_equal 0, bob.booty # check that the pirate starts with 0 booty
  end

  def test_a_pirate_gets_100_booty_for_robbing_ships

    roberts = Pirate.new("The Dread Pirate Roberts")# create a pirate
    roberts.plunder
    roberts.plunder# rob some ships
    roberts.plunder
    assert_equal 300, roberts.booty
