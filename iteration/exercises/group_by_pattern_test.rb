...
class GroupByPatternTest < Minitest::Test
  ...
  def test_group_words_by_first_letter
    ...
    words.each {|word| words_by_first_letter[word[0]] << word}# Your code goes here
  ...
  def test_group_words_by_uniqueness
    ...
    grouped = Hash.new {|hash, key| hash[key] = []}
    words.each {|word| grouped[word.downcase] << word}# Your code goes here
  ...
  def test_group_by_number_of_zeros
    ...
    grouped = Hash.new {|hash, key| hash[key] = []}
    numbers.each {|number| grouped[number.digits.count(0)] << number}# Your code goes here
  ...
  def test_group_by_order_of_magnitude
    ...
    grouped = Hash.new {|key, value| key[value] = []}
    numbers.each {|number| grouped[number.digits.length] << number}# Your code goes here
