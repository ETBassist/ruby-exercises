...
class AllPatternTest < Minitest::Test
  ...
  def test_6
    ...
    all_busy = true
    friend_status.each {|name, status| all_busy = false if status != :busy}# Your Code Here
  ...
  def test_7
    ...
    zip_codes.each {|code| all_five_digits = false if code.digits.count != 5}
  ...
  def test_8
    ...
    # Your Code Here
    all_five_digits = true
    zip_codes.each {|name, code| all_five_digits = false if code.digits.count != 5}
    assert_equal false, all_five_digits
  end
  ...
  def test_9
    ...
    # Your Code Here
    all_caps = true
    snacks.each {|snack| all_caps = false if snack.upcase != snack}
  ...
  def test_10
    ...
    # Your Code Here
    all_caps = true
    snacks.each {|taste, name| all_caps = false if name.upcase != name}
    assert_equal false, all_caps
  end
end
